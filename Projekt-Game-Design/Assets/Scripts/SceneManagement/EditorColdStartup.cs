	...
	public class EditorColdStartup : MonoBehaviour {
		...
		private void Start() {
			if ( persistentManagersSO is { } ) {
				...
				if ( !SceneManager.GetSceneByName(persistentManagersSO.sceneReference.editorAsset.name)
					.isLoaded ) {
					Debug.Log(
						"EditorColdStartup > Start \ncold startup: persistant managers NOT loaded -> send loading event now loading now");
		...
		private void ReloadScene(AsyncOperationHandle<LoadEventChannelSO> obj) {
			...
			SceneManager.UnloadSceneAsync(thisSceneSO.sceneReference.editorAsset.name).completed +=
				operation => {
