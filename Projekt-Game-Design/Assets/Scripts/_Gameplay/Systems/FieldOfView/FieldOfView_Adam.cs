...
namespace FieldOfView {
    // source: http://www.adammil.net/blog/v125_Roguelike_Vision_Algorithms.html#c845
    ...
    public class FieldOfView_Adam : Visibility {
                ...
                x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
            {
                ...
                else // top < 1
                {
                        ...
                        origin)) // if the tile blocks light (i.e. is a wall)...
                    {
                        ...
                        if (top.GreaterOrEqual(topY * 2 + 1, x * 2) &&
                            !BlocksLight(x, topY + 1, octant, origin)) topY++;
                    }
                    ...
                    y--) // use a signed comparison because y can wrap around when decremented
                {
                        ...
                        rangeLimit) // skip the tile if it's out of visual range
                    {
                        ...
                        // bool isVisible =
                        //     isOpaque || ((y != topY || top.Greater(y * 4 - 1, x * 4 + 1)) &&
