...
namespace UI.Components.Tooltip
{
		...
		public class TooltipElement : VisualElement, UpdatedClass
		{
				...
				public TooltipElement(VisualElement tooltipParent)
				{
						...
						UpdateHelper helper = UpdateHelper.FindInstance();
						if ( helper )
								helper.Subscribe(this);
						else
								Debug.LogError("No Update helper found. Cannot use tooltip element. ");
						...
						if ( tooltipLayer != null )
								...
								Debug.LogError("No tooltip layer found. ");
				}
				...
				private void UpdatePosition()
				{
						...
						if ((onTop && parentYPos + resolvedStyle.height < tooltipLayer.worldBound.height)|| posForBottom < 0)
						...
						float xPos = tooltipParent.worldBound.x - 0.5f * ( resolvedStyle.width - tooltipParent.resolvedStyle.width );
