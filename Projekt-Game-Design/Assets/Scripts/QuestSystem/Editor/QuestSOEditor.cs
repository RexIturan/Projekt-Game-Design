...
namespace QuestSystem.Editor {
	...
	public class QuestSOEditor : UnityEditor.Editor {
		...
		private void OnEnable() {  
			so = serializedObject;
			var testTaskProperty = so.FindProperty("testTasks");
			reorderableList = new ReorderableList(so, testTaskProperty, true, true, true, true);
			SetupTaskList(reorderableList);
		}
